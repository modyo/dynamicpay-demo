      ...
      return (
        <div className="plan-item bg-white flex-1 mx-lg-4 mb-4 mb-lg-0">
          ...
          <div className="pricing-price">
            {e.Price !== "Free" ? <span className="sign">$</span> : ''}{e.Price} <span>/mon</span>
          </div>
          <div className="items p-5" dangerouslySetInnerHTML={{ __html: e.Features }} />
          <div className="action p-5"><a className="btn btn-primary" href={e.Link}>{e.Button}</a></div>
        </div>
    )})
  }
  ...
  render() {
    return (
        ...
        <div className="plans-list">
            {this.state.plans ? (<div className="my-5">
          <div className="container">
            <div className="d-lg-flex">{this.planItem()}  </div>
          </div>
        </div>) : <Loading title="Cargando..." />}
        </div>
      </Fragment>
    )
  }
